...
async function run() {
	...
	puppet.on("puppetWillDelete", slack.stopClient.bind(slack));
	...
	puppet.on("receipt", slack.handleMatrixReceipt.bind(slack));
	...
	puppet.setCheckRegisteredHook(slack.checkIfAlreadyRegistered.bind(slack))

...
export class Provisioner {
	...
	private provisioningMutex: Mutex;
	constructor(
		private bridge: PuppetBridge,
	) {
		this.puppetStore = this.bridge.puppetStore;
		this.provisioningMutex = new Mutex();
	}
	...
	public async new(puppetMxid: string, data: IPuppetData, userId?: string): Promise<number | undefined> {
		if (!this.canCreate(puppetMxid)) {
			return -1;
		}
		let puppetId = -1;

		log.info(`Entering new puppet provisioningMutex`);
		await this.provisioningMutex.runExclusive(async () => {
			if (this.bridge.hooks.checkRegisteredHook && await this.bridge.hooks.checkRegisteredHook(data)) return undefined;
			...
			puppetId = await this.puppetStore.new(puppetMxid, data, userId, isGlobal);
			log.info(`Created new puppet with id ${puppetId}`);
			this.bridge.emit("puppetNew", puppetId, data);

			log.info(`Leaving new puppet provisioningMutex`);
		});

		this.adjustMuteListRooms(puppetId, puppetMxid);
		return puppetId;
	}
	...
	public async delete(puppetMxid: string, puppetId: number) {
		...
		await this.bridge.setBridgeStatus("LOGGED_OUT", puppetId)
		this.bridge.emit("puppetWillDelete", puppetId);

...
export class Util {
	...
	public static async ProcessProfileUpdate(
		...
		log.verbose("Processing profile update...");
		...
		if (oldProfile === null) {
			if (checkName) {
				log.info("Updating new display name")
		...
		if (checkName !== undefined && checkName !== null && checkName !== oldProfile.name) {
			log.info("Updating old display name")
			result.name = checkName;
		} else if (checkName === undefined || checkName === null) {
			log.info("check name does not exist so no display name update needed")
		}

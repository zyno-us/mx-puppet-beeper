...
export class BotProvisioner {
	...
	public async processEvent(roomId: string, event: MessageEvent<TextualMessageEventContent>) {
		...
		switch (fnCollect ? "link" : arg) {
			case "relink":
			case "link": {
				let puppetId: number | undefined = -1;
				...
				if (puppetId === -1) {
					...
					if (!puppetId) {
						await this.sendMessage(roomId, `Duplicate Puppet already registered`);
						return;
					}
	...
	public async sendStatusMessage(room: number | IRemoteRoom, msg: string) {
		...
		sendStr += msg;
		// Brad: Don't actually send the message, as it's confusing folks. Just log it.
		log.info(sendStr);
...
Usage: \`fixmute <room resolvable>\``,
		...
		this.registerCommand("fixmuteall", {
			fn: async (sender: string, param: string, sendMessage: SendMessageFn) => {
				await sendMessage("Fixing muted rooms...");
				const entries = await this.bridge.roomStore.getAll();
				for (const entry of entries) {
					await this.provisioner.adjustMuteIfInRoom(sender, entry.mxid);
				}
				await sendMessage("All muted rooms fixed");
			},
			help: `Fix the power levels according to puppet & relay availability in all bridged rooms`,
			withPid: false,
			inRoom: false,
		})

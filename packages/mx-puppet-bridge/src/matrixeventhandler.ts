		...
		for (const [eventId, allContents] of Object.entries<any>(rawEvent.content)) {
			if (allContents["m.read"]) {
				...
				for (const [userId, content] of Object.entries<any>(allContents["m.read"])) {
					...
					// Ported over from MatrixClientSyncroniser to replace sync read receipts
					const lastEvent = this.lastReceiptForRoom.get(room.roomId);
					if (!lastEvent || lastEvent.ts < content.ts) {
						this.lastReceiptForRoom.set(room.roomId, { ts: content.ts, eventId });
					}
					this.bridge.delayedFunction.set(`receipt-${room.roomId}`, async () => {
						...
						log.info(`MatrixEventHandler: Send receipts for room: ${room.roomId} for event ${lastEventId}`);

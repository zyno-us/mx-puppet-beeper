...
//import { MatrixClientSyncroniser } from "./matrixclientsyncroniser";
...
import { ProvisioningAPI, EndpointFn } from "./provisioningapi";
...
import {
	...
	UserExistsHook, RoomExistsHook, GroupExistsHook, ResolveRoomIdHook, IEventInfo, MatrixPresence, GetGroupInfoHook, CheckRegisteredHook
} from "./interfaces";
...
export interface IPuppetBridgeHooks {
	...
	checkRegisteredHook?: CheckRegisteredHook;
}
...
export class BridgeMetrics {
	...
	public tokenError: prometheus.Counter<string>;
	public tokenLost: prometheus.Counter<string>;
	public incomingMessages: prometheus.Counter<string>;
	public addingGhosts: prometheus.Counter<string>;
}
...
export class PuppetBridge extends EventEmitter {
	...
	public async init() {
		...
		this.bridgeStatus = new BridgeStatus(this);
		...
		if (this.config.metrics.enabled) {
			// Brad: Turn default metrics (like CPU, GC frequency, etc) off to take some strain off
			// of prometheus, we never look at these metrics.
	...
	public async start(callback?: () => Promise<void>) {
		...
		process.on("unhandledRejection", (err, promise) => {
			const newErr = err as any;
			log.error("Unhandled Promise rejection", newErr.body || err)
			//log.error(promise);
		});
		...
		if (displayname) {
			try {
				const res = await this.appservice.botIntent.underlyingClient.getDisplayName();
				oldDisplayName = res.displayname;
			} catch {}
		}
		...
		//await this.matrixClientSyncroniser.init(puppets);
		this.metrics.puppet.set({protocol: this.protocol.id}, puppets.length);
		if (puppets.length === 0) {
			await this.setBridgeStatus("UNCONFIGURED")
		} else {
			for (const p of puppets) {
				await this.setBridgeStatus("CONNECTING", p.puppetId)
			}
		}
		for (const p of puppets) {
	...
	public setCheckRegisteredHook(hook: CheckRegisteredHook) {
		this.hooks.checkRegisteredHook = hook;
	}
	 ...
	 */
	public async updateRoom(room: IRemoteRoom, doCreate: boolean = false) {
		log.verbose("Got request to update a room");
		await this.roomSync.getMxid(room, undefined, doCreate);
	}
	...
	/**
	 * Specific for NovaChat method to archive/unarchive room
	 */
	public async toggleRoomArchived(puppetId: number, mxid: string, archived: boolean) {
		// tslint:disable-next-line:no-magic-numbers
		const defEventsLevel = archived ? 50 : 0;
		const archivedTagName = "m.lowpriority";

		const powerLevels = await this.botIntent.underlyingClient.getRoomStateEvent(mxid, "m.room.power_levels", "");
		if (powerLevels.events_default && powerLevels.events_default === defEventsLevel) {
			// ok, room already has this state
			return;
		}
		powerLevels.events_default = defEventsLevel;
		if (archived) {
			try {
				await this.addTagToRoom(puppetId, mxid, archivedTagName);
			} catch {}
			try {
				await this.botIntent.underlyingClient.sendStateEvent(mxid, "m.room.power_levels", "", powerLevels);
			} catch {}
		} else {
			try {
				await this.botIntent.underlyingClient.sendStateEvent(mxid, "m.room.power_levels", "", powerLevels);
			...
			try {
				await this.removeTagFromRoom(puppetId, mxid, archivedTagName);
			} catch {}
		}
	}
	/**
	 * Adds tag from matrix room if using double-puppeting, given room mxid
	 */
	public async addTagToRoom(puppetId: number, mxid: string, tagName: string) {
		const client = await this.userSync.getPuppetClient(puppetId);
		if (client) {
			const res = await client.getRoomTags(mxid);
			const hasTag = res && res.tags && Object.keys(res.tags).includes(tagName);
			if (!hasTag) {
				await client.setRoomTag(mxid, tagName);
			}
		}
	}

	/**
	 * Remove tag from matrix room if using double-puppeting, given room mxid
	 */
	public async removeTagFromRoom(puppetId: number, mxid: string, tagName: string) {
		const client = await this.userSync.getPuppetClient(puppetId);
		if (client) {
			const res = await client.getRoomTags(mxid);
			const hasTag = res && res.tags && Object.keys(res.tags).includes(tagName);
			if (hasTag) {
				await client.deleteRoomTag(mxid, tagName);
			}
		}
	}

	/**
	 * get mxid for remote room if it exist
	 */
	public async getRoomMxidIfExist(data: IRemoteRoom): Promise<string | null> {
		const dbPuppetId = await this.namespaceHandler.getDbPuppetId(data.puppetId);
		const room = await this.roomStore.getByRemote(dbPuppetId, data.roomId);
		if (!room) {
			return null;
		}
		return room.mxid;
	 ...
	 */
	public async addUsers(room: IRemoteRoom) {
		await this.roomSync.addGhosts(room);
	}
	...
	/**
	 * Get id for last handled event in remote room
	 */
	public async getLastRemoteEvent(room: IRemoteRoom) {
		return await this.eventSync.getLastRemote(room);
	}
	...
	/**
	 * Registers a custom GET endpoint for the Provisioning API
	 */
	public registerProvisioningGetEndpoint(path: string, command: EndpointFn) {
		this.provisioningAPI.registerProvisioningGetEndpoint(path, command);
	}
	/**
	 * Registers a custom POST endpoint for the Provisioning API
	 */
	public registerProvisioningPostEndpoint(path: string, command: EndpointFn) {
		this.provisioningAPI.registerProvisioningPostEndpoint(path, command);
	}
	...
	/**
	 * Send a file to matrix as a reply, auto-detect its type
	 */
	public async sendReplyFileDetect(params: IReceiveParams, replyTs: string, thing: string | Buffer, name?: string) {
		await this.remoteEventHandler.sendReplyFileByType("detect", replyTs, params, thing, name);
	}
	...
	/**
	 * Sets the bridges status
	 */
	public async setBridgeStatus(bridgeStatus: string, puppetId?: number, errorCode?: string) {
		await this.bridgeStatus.send(bridgeStatus, puppetId, errorCode);
	}
	...
	public async redactEvent(client: MatrixClient, roomId: string, eventId: string) {
		try {
			await client.redactEvent(roomId, eventId, null, null, {source: this.protocol.id});
		} catch (err) {
			if (err.body.errcode === "M_FORBIDDEN") {
				...
				await opClient.redactEvent(roomId, eventId, null, null, {source: this.protocol.id});
			} else {
				throw err;
			}
		}
	...
	/**
	 * Sends m.direct flag information
	 */
	 public async sendMDirect(puppetId: number) { 
		await this.roomSync.sendMDirect(puppetId);
	 }

...
export class DbEventStore {
	...
	public async getLastRemote(puppetId: number, roomId: string): Promise<string | null> {
		const row = await this.db.Get(
			"SELECT * FROM event_store WHERE puppet_id = $p AND room_id = $room ORDER BY remote_id DESC LIMIT 1", {
				p: puppetId,
				room: roomId,
		});
		if (!row) {
		...
		return row.remote_id as string;
	}

	public async getRemoteAfter(puppetId: number, roomId: string, fromRemoteId: string): Promise<string[]> {
		const result: string[] = [];
		const rows = await this.db.All(
			"SELECT * FROM event_store WHERE puppet_id = $p AND room_id = $room AND remote_id > $from", {
				p: puppetId,
				room: roomId,
				from: fromRemoteId,
			}

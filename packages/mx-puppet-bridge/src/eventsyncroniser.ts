...
export class EventSyncroniser {
	...
	public async insert(room: IRemoteRoom, matrixId: string, remoteId?: string) {
		...
		try {
			...
			log.info("Failed to send read reciept", err);
		}
	}
	...
	public async getLastRemote(room: IRemoteRoom): Promise<string | null> {
		const dbPuppetId = await this.bridge.namespaceHandler.getDbPuppetId(room.puppetId);
		return await this.eventStore.getLastRemote(dbPuppetId, room.roomId);
	}

	public async getRemoteAfter(room: IRemoteRoom, fromRemoteId: string): Promise<string[]> {
		const dbPuppetId = await this.bridge.namespaceHandler.getDbPuppetId(room.puppetId);
		return await this.eventStore.getRemoteAfter(dbPuppetId, room.roomId, fromRemoteId);
	}
}

...
export class Store {
	...
	public async init(
		...
		log.info(`DB Init Params: overrideSchema: ${overrideSchema} table: ${table} openDatabase: ${openDatabase}`);
		if (openDatabase) {
			await this.openDatabase();
		}
		log.info("DB should be open here! DB: ", this.db.type);
	...
	private async getSchemaVersion(table: string = "schema", retryNum: number = 0): Promise<number> {
		log.info(`_get_${table}_version`);
		...
		try {
			...
			log.info(`Fetching schema version, versionReply:`, versionReply);
			version = Number(versionReply!.version);
			log.info(`Fetching schema version, version: ${version}`);
		} catch (err) {
			log.warn("Schema version fetching error:", err);
			if (isPGError(err) && err.code === "42P01") {
				log.warn("Schema version doesn't exist, safely assuming version 0");
			  return 0;
			}
			if (retryNum < 10) {
				const retrySeconds = (retryNum + 1) * 2;
				log.warn(`Retrying version fetch (waiting ${retrySeconds} seconds, current try ${retryNum})`);
				await Util.sleep(retrySeconds * 1000);
				return await this.getSchemaVersion(table, retryNum + 1);
			} else {
				log.error("Fetching schema retries failed, aborting");
				process.exit(-1);
			}
		}
		return version;

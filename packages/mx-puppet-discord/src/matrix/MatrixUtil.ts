...
export class MatrixUtil {
	...
	public getRemoteRoom(puppetId: number, channel: BridgeableChannel): IRemoteRoom {
		...
		if (channel instanceof Discord.GroupDMChannel) {
			const members = [...channel.recipients.values()];
			const currentId = this.app.puppets[puppetId].data.id;
			const nameVarsTemp: IStringFormatterVars = {};
			nameVarsTemp.name = members
			...
			if (!channel.name) {
				ret.nameVars = nameVarsTemp;
			} else {
				ret.nameVars = { name: channel.name };
			}
			ret.isDirect = true;

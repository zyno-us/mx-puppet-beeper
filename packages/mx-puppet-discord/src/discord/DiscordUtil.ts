...
export class DiscordUtil {
	...
	public async getOrCreateWebhook(chan: TextGuildChannel): Promise<Discord.Webhook | null> {
		...
		try {
			...
			try {
				// Compeletely disable webhook creation
				// (Callers to getOrCreateWebhook always check for null, so this should be okay)
				log.warn("Attempt to create webhook, webhook creation has been removed");
				return null;

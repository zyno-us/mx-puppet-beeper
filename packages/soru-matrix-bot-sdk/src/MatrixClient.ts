...
export class MatrixClient extends EventEmitter {
    ...
    protected async startSync(emitFn: (emitEventType: string, ...payload: any[]) => Promise<any> = null) {
        ...
        const promiseWhile = async () => {
            ...
            try {
                ...
                LogService.error("MatrixClientLite", e.body || e);
            }
    ...
    protected doSync(token: string): Promise<any> {
        ...
        // timeout is 1min if we have a token, otherwise 10min
        return this.doRequest("GET", "/_matrix/client/r0/sync", conf, null, (token ? 60000 : 600000));
    }
    ...
    public async setDisplayName(displayName: string): Promise<any> {
        const userId = encodeURIComponent(await this.getUserId());
        // this request can be very slow https://github.com/matrix-org/synapse/issues/1297
     ...
     * @param {string} userId optional to get display name for the another user
     * @returns {Promise<any>} resolves when complete
     */
    @timedMatrixClientFunctionCall()
    public async getDisplayName(userId?: string): Promise<any> {
        if (!userId) {
            userId = await this.getUserId();
        }
        return this.doRequest("GET", "/_matrix/client/r0/profile/" + encodeURIComponent(userId) + "/displayname");
    }
     ...
     * @returns {Promise<any>} resolves when forgot
     */
    @timedMatrixClientFunctionCall()
    public forgetRoom(roomId: string): Promise<any> {
        return this.doRequest("POST", "/_matrix/client/r0/rooms/" + encodeURIComponent(roomId) + "/forget");
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public replyText(roomId: string, event: any, text: string, html: string = null, qs?: object): Promise<string> {
        if (!html) html = htmlEncode(text);

        const reply = RichReply.createFor(roomId, event, text, html);
        return this.sendMessage(roomId, reply, qs);
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public replyNotice(roomId: string, event: any, text: string, html: string = null, qs?: object): Promise<string> {
        if (!html) html = htmlEncode(text);
     ...
     */
    @timedMatrixClientFunctionCall()
    public sendNotice(roomId: string, text: string, qs?: object): Promise<string> {
     ...
     */
    @timedMatrixClientFunctionCall()
    public sendText(roomId: string, text: string, qs?: object): Promise<string> {
        return this.sendMessage(roomId, {
            body: text,
            msgtype: "m.text",
        }, qs);
     ...
     */
    @timedMatrixClientFunctionCall()
    public sendMessage(roomId: string, content: any, qs?: object | null): Promise<string> {
        return this.sendEvent(roomId, "m.room.message", content, qs);
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public sendEvent(roomId: string, eventType: string, content: any, qs: object | null = null): Promise<string> {
        const txnId = (new Date().getTime()) + "__REQ" + this.requestId;
        return this.doRequest("PUT", "/_matrix/client/r0/rooms/" + encodeURIComponent(roomId) + "/send/" + encodeURIComponent(eventType) + "/" + encodeURIComponent(txnId), qs, content).then(response => {
     ...
     */
    @timedMatrixClientFunctionCall()
    public sendStateEvent(roomId: string, type: string, stateKey: string, content: any, qs: object | null = null): Promise<string> {
        return this.doRequest("PUT", "/_matrix/client/r0/rooms/" + encodeURIComponent(roomId) + "/state/" + encodeURIComponent(type) + "/" + encodeURIComponent(stateKey), qs, content).then(response => {
            return response['event_id'];
        });
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public redactEvent(roomId: string, eventId: string, reason: string | null = null, qs: object | null = null, body: object | null = null): Promise<string> {
        const txnId = (new Date().getTime()) + "__REQ" + this.requestId;
        const reasonContent = reason !== null ? {reason} : {};
        const content = {
            ...reasonContent,
            ...body,
        }
        return this.doRequest("PUT", `/_matrix/client/r0/rooms/${encodeURIComponent(roomId)}/redact/${encodeURIComponent(eventId)}/${txnId}`, qs, content).then(response => {
            return response['event_id'];
        });
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public async getRoomTags(roomId: string): Promise<any> {
        const userId = encodeURIComponent(await this.getUserId());
        roomId = encodeURIComponent(roomId);
        return this.doRequest("GET", `/_matrix/client/r0/user/${userId}/rooms/${roomId}/tags/`);
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public async setRoomTag(roomId: string, tagName: string): Promise<any> {
        const userId = encodeURIComponent(await this.getUserId());
        roomId = encodeURIComponent(roomId);
        tagName = encodeURIComponent(tagName);
        return this.doRequest("PUT", `/_matrix/client/r0/user/${userId}/rooms/${roomId}/tags/${tagName}`, null, {});
    }
     ...
     */
    @timedMatrixClientFunctionCall()
    public async deleteRoomTag(roomId: string, tagName: string): Promise<any> {
        const userId = encodeURIComponent(await this.getUserId());
        roomId = encodeURIComponent(roomId);
        tagName = encodeURIComponent(tagName);
        return this.doRequest("DELETE", `/_matrix/client/r0/user/${userId}/rooms/${roomId}/tags/${tagName}`);
    }
     ...
     * @param {any} newHeaders The headers to add to the query.
     * @returns {Promise<any>} Resolves to the response (body), rejected if a non-2xx status code was returned.
     */
    @timedMatrixClientFunctionCall()
    public doRequest(method, endpoint, qs = null, body: string | Buffer | stream.Readable | object | null = null, timeout = 60000, raw = false, contentType = "application/json", noEncoding = false, newHeaders = {}): Promise<any> {
        ...
        for (let newHeader in newHeaders) {
            headers[newHeader] = newHeaders[newHeader];
        }

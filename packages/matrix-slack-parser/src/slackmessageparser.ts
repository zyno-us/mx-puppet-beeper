...
export class SlackMarkdownParser {
	public async parseMarkdown (
		...
		let content = markdown.toHTML(str, {
			...
			slackCallbacks: this.getSlackParseCallbacks(opts, mdOpts),
	...
	private getSlackParseCallbacks (
		opts: ISlackMessageParserOpts,
		mdOpts: ISlackMarkdownParserOpts,
	) {
		return {
	...
	private async parseBlock(
		...
		switch (block.type) {
			...
			case "section": {
				...
				let retStr = "";
				if (section.text) {
					retStr += await this.parseElement(opts, section.text);
				}
			...
			case "context": {
				...
				for (const [index, element] of context.elements.entries()) {
					...
					if (index !== context.elements.length - 1) {
						retStr += " ";
					}
				}
				return retStr;

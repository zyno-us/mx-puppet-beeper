...
export class User extends IconBase {
	...
	public async checkIm():  Promise<Channel | null> {
		if (this.imChannel) {
			return this.imChannel;
		}
		let reply;
		try {
			reply = await this.client.web(this.team.id).users.conversations({
				types: "im",
				user: this.id,
			})
		} catch {
			log.warn("Problem with team for checkIm: ", this.team.id);
			return null;
		}

		if (!reply || !reply.ok || !reply.channels) {
			return null;
		}
		const replyData = reply.channels as any;
		log.info("trying to check im");
		try {
			const chanData = replyData[0] as IChannelData;
			log.info("chan data id", chanData.id)
			let chan = this.client.getChannel(chanData.id, this.team.id);
			log.info("check chan");
			if (chan) {
				log.info("chan name", chan.name, "and chan id",  chan.id)
				this.imChannel = chan;
				return chan;
			}
			return null;

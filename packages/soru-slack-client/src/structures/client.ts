...
import { IMessageData, Message } from "./message";
...
export class Client extends EventEmitter {
	...
	public async addToken(token: string, teamId: string = "", userId: string = "") {
		...
		const reconnect = async () => {
			...
			for (let i = 0; i < NUM_ATTEMPTS; ++i) {
				try {
					log.info("trying to reconnect, ",  NUM_ATTEMPTS - i)
					...
					log.error("Failed to re-start RTM client", err, NUM_ATTEMPTS - i - 1, "tries left");
				}
				if (i == NUM_ATTEMPTS - 1) {
					log.error("Failed to re-start RTM client, no tries left")
		...
		return new Promise(async (resolve, reject) => {
			...
			rtm.on("authenticated", async (data) => {
				log.info("RTM event: authenticated");
			...
			rtm.on("ready", () => {
				log.info("RTM event: ready");
				resolve();
			});

			rtm.on("connected", () => {
				log.info("RTM event: connected")
			})
			...
			if (!this.opts.events) {
				for (const ev of ["im_created", "channel_created", "channel_rename", "group_rename", "channel_joined", "group_joined"]) {
					const [, type] = ev.split("_");
					const eventName = `channel${Util.FirstLetterToUpperCase(type)}`;
					rtm.on(ev, (data) => {
						log.debug(`RTM event: ${ev}`);
						data.channel.team_id = teamId;
						this.addChannel(data.channel);
						const chanObj = this.getChannel(data.channel.id, teamId);
						// you may need to handle events by type
						// addChannel/changeChannel events will be emit, but listen them is redundantly in this case
						this.emit(eventName, chanObj);
					});
				}

				for (const ev of ["channel_left", "group_left"]) {
					rtm.on(ev, (data) => {
						log.debug(`RTM event: ${ev}`);
						const chanObj = this.getChannel(data.channel, teamId);
						if (chanObj) {
							chanObj.isMember = false;
							this.emit("channelLeft", chanObj);
						}
					});
				}

				for (const ev of ["channel_unarchive", "group_unarchive"]) {
					rtm.on(ev, (data) => {
						log.debug(`RTM event: ${ev}`);
						const chanObj = this.getChannel(data.channel, teamId);
						if (chanObj) {
							chanObj.archived = false;
							this.emit("channelUnarchive", chanObj);
						}
					});
				}

				for (const ev of ["channel_archive", "group_archive"]) {
					rtm.on(ev, (data) => {
						log.debug(`RTM event: ${ev}`);
						const chanObj = this.getChannel(data.channel, teamId);
						if (chanObj) {
							chanObj.archived = true;
							this.emit("channelArchive", chanObj);
						}
					});
				}

				for (const ev of ["group_open", "im_open"]) {
					rtm.on(ev, (data) => {
						log.debug(`RTM event: ${ev}`);
						const chanObj = this.getChannel(data.channel, teamId);
						if (chanObj) {
							if (chanObj.opened && !chanObj.partial) {
								return;
							}
							chanObj.opened = true;
							this.emit("channelOpen", chanObj);
						}
					});
				}

				for (const ev of ["group_close", "im_close"]) {
					rtm.on(ev, (data) => {
						log.debug(`RTM event: ${ev}`);
						const chanObj = this.getChannel(data.channel, teamId);
						if (chanObj) {
							...
							chanObj.opened = false;
							this.emit("channelClose", chanObj);
						}
					});
				}
				...
				rtm.on("member_joined_channel", (data) => {
					if (this.isActiveUser(teamId, data.user)) {
						return;
					}
				...
				for (const ev of ["channel_marked", "group_marked", "im_marked"]) {
					rtm.on(ev, (data) => {
						log.debug(`RTM event: ${ev}`);
						const chanObj = this.getChannel(data.channel, teamId);
						const userObj = this.getActiveUser(teamId);
						if (chanObj && !chanObj.isAlreadyRead(data.ts)) {
							log.info("marking channel as read at timestamp", data.ts)
							chanObj.lastRead = data.ts;
							this.emit("channelMarked", chanObj, userObj, data.ts);
						}
					});
				}
			}
			...
			try {
				await rtm.start();
				log.info("RTM connected and ready");
			} catch (err) {
				log.error("Failed to start rtm client", err);
				reject(err);
			}
	...
	public async connectToEventsApi() {
		...
		for (const ev of ["im_created", "channel_created", "channel_joined", "group_joined", "channel_rename", "group_rename"]) {
			this.events.on(ev, (data, evt) => {
				if (evt.api_app_id !== appId) {
					return;
				}
				log.debug(`Events event: ${ev}`);
	...
	public getActiveUser(teamId: string): User | null {
		const userId = this.rtm(teamId).activeUserId;
		if (!userId) {
			throw new Error("Active user not found");
		}
		return this.getUser(userId, teamId);
	}
	...
	public async getChannelHistory(
		...
		const useCursor = limit > 1000;
		const iterLimit = useCursor ? 1000 : limit;
		do {
			const ret = await this.web(teamId).conversations.history({
				channel: channel.id,
				iterLimit,
				oldest,
				latest,
			...
			if (!ret || !ret.ok || !ret.messages) {
				throw new Error("Bad response");
			}
			for (const data of ret.messages as IMessageData[]) {
				let author: User | Bot | null = null;
				if (data.user) {
					author = this.getUser(data.user, data.team || teamId);
					if (!author) {
						this.addUser({
							id: data.user,
							team_id: teamId,
					...
					// @ts-ignore
					this.addBot(Object.assign(data, { team_id: data.team || teamId }));
					author = this.getBot(data.bot_id, teamId);
				}
				if (!author) {
					log.warn("Author not found. Ignore message");
					continue;
				}
				if ([
					"channel_join",
	...
	public async getChannelAndAuthor(data): Promise<{ channel: Channel, author: User | Bot }> {
		...
		let channel = this.getChannel(channelId, teamId);
		if (!channel) {
			log.info("channel not found, checking slack")
			const chan = await this.web(sourceTeamId).conversations.info({
				channel: channelId,
				include_num_members: true,
			});
			if (!chan || !chan.ok || !chan.channel) {
				throw new Error("Channel not found");
			}
			const channelInfo = chan.channel as IChannelData;
			this.addChannel(channelInfo);
			channel = this.getChannel(channelId, teamId)
			if (!channel) {
				throw new Error("Channel not recovered");
			}
	...
	private async handleMessageEvent(data) {
		if ([
			...
			// TODO add handler to send status messages
			return;
		}
...
<body>
	...
	private isActiveUser(teamId: string, userId: string): boolean {
		return this.rtm(teamId).activeUserId === userId;
	}
}
